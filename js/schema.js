...
const SCHEMA = {
    ...
    libraries: {
        '热梗素材库': {
            ...
            headers: ['编号','类型标签','来源平台','核心冲突点','情感层次','改编方向','甜虐指数','甜虐平衡指数','受众偏好','热度等级','主流适配平台','适配角色关系','适用章节','道具符号','禁忌红线','多平台热度','伏笔需求','读者预期','风险提示','适用情节','绑定作品','收藏'],
            displayFields: ['编号','类型标签','核心冲突点','改编方向','甜虐指数','受众偏好','热度等级','来源平台'],
            filterFields: ['类型标签','来源平台','受众偏好','热度等级','适配角色关系','绑定作品']
        },
        '冲突素材库': {
            prefix: 'CT',
            icon: '⚔️',
            ...
            headers: ['编号','冲突类型','冲突快捷标签','来源平台','现实原型','核心矛盾','情绪曲线','情绪起伏节点','情绪价值','适配角色关系','适用场景','风险等级','改编方向','改编案例','爆发场景','解决方式','伏笔需求','适用情节','绑定作品','收藏'],
            displayFields: ['编号','冲突类型','冲突快捷标签','核心矛盾','情绪曲线','风险等级','适用场景'],
            filterFields: ['冲突类型','冲突快捷标签','风险等级','适用场景','适配角色关系','绑定作品']
        },
        '钩子素材库': {
            prefix: 'GZ',
            icon: '🪝',
            ...
            headers: ['编号','钩子类型','埋设位置','类型标签','引爆章节','核心元素','埋设手法','情感导向','回收周期','回收章节','关联反转','钩子密度','伏笔要求','修改方向','风险提示','适用情节','绑定作品','收藏'],
            displayFields: ['编号','钩子类型','核心元素','引爆章节','回收周期','回收章节','情感导向'],
            filterFields: ['钩子类型','埋设手法','钩子密度','情感导向','绑定作品']
        },
        '反转素材库': {
            prefix: 'FZ',
            icon: '🔄',
            ...
            headers: ['编号','反转类型','铺垫线索','爆发章节','情感冲击','逻辑校验','表面呈现','真相揭露','伏笔设计','改编案例','现实原型','适用题材','风险屏障','适用情节','绑定作品','收藏'],
            displayFields: ['编号','反转类型','爆发章节','情感冲击','真相揭露','适用题材'],
            filterFields: ['反转类型','情感冲击','适用题材','绑定作品']
        },
        '人设基因库': {
            prefix: 'RS',
            icon: '🧬',
            ...
            headers: ['编号','角色定位','性格标签','标签','职业/身份','情感障碍','情感心结诱因','专属物品','标志动作','感情线伏笔','隐秘关联','荷尔蒙触发点','读者心动戳点','致命性缺点','救赎开关','禁忌边界','适用情节','绑定作品','收藏'],
            displayFields: ['编号','角色定位','性格标签','职业/身份','读者心动戳点','专属物品','适用情节'],
            filterFields: ['角色定位','性格标签','标签','适用情节','绑定作品']
        },
        '场景库': {
            prefix: 'CJ',
            icon: '🎬',
            ...
            headers: ['编号','场景类型','视觉焦点','听觉细节','触觉意象','嗅觉记忆','味觉隐喻','情感曲线','冲突触发点','伏笔回收点','关联素材ID','适用情节','数据参考','禁忌提示','情感强度','绑定作品','收藏'],
            displayFields: ['编号','场景类型','视觉焦点','情感曲线','冲突触发点','适用情节'],
            filterFields: ['场景类型','适用情节','绑定作品']
        },
        '法律风险库': {
            prefix: 'FL',
            icon: '⚖️',
            ...
            headers: ['编号','风险类型','法律依据','触发条件','后果严重性','现实原型','改编方案','预防措施','戏剧化技巧','适用情节','绑定作品','收藏'],
            displayFields: ['编号','风险类型','法律依据','后果严重性','改编方案','适用情节'],
            filterFields: ['风险类型','后果严重性','绑定作品']
        },
        '词汇库': {
            prefix: 'CH',
            icon: '📝',
            ...
            headers: ['编号','分类','子类','核心词汇','强度','通感转化示例','适用题材','关联情绪','出处','绑定作品','收藏'],
            displayFields: ['编号','分类','子类','核心词汇','强度','通感转化示例'],
            filterFields: ['分类','子类','适用题材','绑定作品']
        },
        '情绪库': {
            prefix: 'QX',
            icon: '💫',
            ...
            headers: ['编号','核心情绪','强度等级','生理反应','微表情编码','行为映射','对话特征','适用场景','禁忌误用','案例来源','绑定作品','收藏'],
            displayFields: ['编号','核心情绪','强度等级','生理反应','行为映射','适用场景'],
            filterFields: ['核心情绪','适用场景','绑定作品']
        },
        '景色库': {
            prefix: 'JS',
            icon: '🏔️',
            ...
            headers: ['编号','时空坐标','光学描写','声学描写','嗅觉层次','触觉反馈','动态元素','数据层','时代标签','关联色卡','关联素材ID','绑定作品','收藏'],
            displayFields: ['编号','时空坐标','光学描写','声学描写','时代标签','关联色卡'],
            filterFields: ['时代标签','绑定作品']
        },
        '动作库': {
            prefix: 'DZ',
            icon: '✋',
            ...
            headers: ['编号','场景类型','动作分级','主体动作','连带反应','隐喻意义','节奏值','禁忌组合','经典案例','关联素材ID','绑定作品','收藏'],
            displayFields: ['编号','场景类型','动作分级','主体动作','隐喻意义','节奏值'],
            filterFields: ['场景类型','动作分级','绑定作品']
        },
        '对话库': {
            prefix: 'DH',
            icon: '💬',
            ...
            headers: ['编号','冲突类型','表层对话','潜台词','动作锚点','声调标记','信息密度','权力关系','出处章节','关联素材ID','绑定作品','收藏'],
            displayFields: ['编号','冲突类型','表层对话','潜台词','权力关系','出处章节'],
            filterFields: ['冲突类型','权力关系','绑定作品']
        },
        '金句库': {
            prefix: 'JJ',
            icon: '✨',
            ...
            headers: ['编号','金句内容','类型标签','声韵结构','隐喻密度','跨库冲击力','关联热梗','适用情节','关联素材ID','绑定作品','收藏'],
            displayFields: ['编号','金句内容','类型标签','隐喻密度','适用情节'],
            filterFields: ['类型标签','隐喻密度','绑定作品']
        },
        '幽默素材库': {
            prefix: 'HM',
            icon: '😄',
            ...
            headers: ['编号','评论原文','核心笑点','幽默类型','可复用结构','优化建议','情绪强度','适用场景','高频关键词','禁忌提示','参考作品','创作周期','对应章节','埋梗位置','黑色幽默指数','冷笑话指数','绑定作品','收藏'],
            displayFields: ['编号','评论原文','核心笑点','幽默类型','适用场景','黑色幽默指数'],
            filterFields: ['幽默类型','适用场景','绑定作品']
        }
    },
    ...
    nav: [
        ...
        },
        {
            ...
            items: [
                ...
                { id: 'inspiration', name: '灵感收纳库', icon: '💡', badge: 'NEW' }
            ]
        },
        {
            section: '素材库',
