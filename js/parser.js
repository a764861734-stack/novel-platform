...
const Parser = {
    ...
    fieldPatterns: [
        ...
        /^\[(.+?)\]\s*(.+)$/,
        // 直接字段名
        /^(编号|类型标签|来源平台|核心冲突点|情感层次|改编方向|甜虐指数|甜虐平衡指数|受众偏好|热度等级|主流适配平台|适配角色关系|适用章节|道具符号|禁忌红线|多平台热度|伏笔需求|读者预期|风险提示|适用情节|绑定作品|收藏|冲突类型|冲突快捷标签|现实原型|核心矛盾|情绪曲线|情绪起伏节点|情绪价值|适用场景|风险等级|改编案例|爆发场景|解决方式|钩子类型|埋设位置|引爆章节|核心元素|埋设手法|情感导向|回收周期|回收章节|关联反转|钩子密度|伏笔要求|修改方向|反转类型|铺垫线索|情感冲击|逻辑校验|表面呈现|真相揭露|伏笔设计|适用题材|风险屏障|角色定位|性格标签|标签|职业\/身份|职业|身份|情感障碍|情感心结诱因|专属物品|标志动作|感情线伏笔|隐秘关联|荷尔蒙触发点|读者心动戳点|致命性缺点|救赎开关|禁忌边界|场景类型|视觉焦点|听觉细节|触觉意象|嗅觉记忆|味觉隐喻|冲突触发点|伏笔回收点|关联素材ID|数据参考|禁忌提示|情感强度|风险类型|法律依据|触发条件|后果严重性|预防措施|戏剧化技巧|分类|子类|核心词汇|强度|通感转化示例|关联情绪|出处|核心情绪|强度等级|生理反应|微表情编码|行为映射|对话特征|禁忌误用|案例来源|时空坐标|光学描写|声学描写|嗅觉层次|触觉反馈|动态元素|数据层|时代标签|关联色卡|动作分级|主体动作|连带反应|隐喻意义|节奏值|禁忌组合|经典案例|表层对话|潜台词|动作锚点|声调标记|信息密度|权力关系|出处章节|金句内容|声韵结构|隐喻密度|跨库冲击力|关联热梗|评论原文|核心笑点|幽默类型|可复用结构|优化建议|情绪强度|高频关键词|参考作品|创作周期|对应章节|埋梗位置|黑色幽默指数|冷笑话指数)\s*[：:]\s*(.+)$/
    ],
